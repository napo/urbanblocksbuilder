...
This is a heuristic based purely on tags, not on OSM node identity - it cannot know, for instance, that a `bridge=yes` way and its ground-level approach are the same real intersection at the transition point. In practice this is fine: the whole point of logical levels is to keep those two apart topologically (a bridge deliberately does *not* connect to the road passing underneath it, even where they share tags or are geographically close), and connectivity *within* a level is handled by real OSM node identity - see step 7.

**A related, previously-unfixed edge case, now corrected**: a road's ground-level approach can end very close (within `snappingToleranceMeters`, 3 m by default) to the point where it - or an unrelated feature, such as a river - changes logical level, simply because that is where a real bridge or tunnel physically begins. Before OSM node IDs were propagated through the pipeline (see step 7), the graph-construction step had no way to tell "these two points are close because they are meant to connect" apart from "these two points are close purely by coincidence of geography", so the ground-level stub could end up welded onto the unrelated feature instead of being correctly pruned as a dead end in step 8. This was reproduced on a real bridge-over-river crossing (a road's pre-bridge approach snapping onto the river's own centreline). Graph construction is now keyed by the real OSM node ID when one is known, so two vertices are only ever treated as the same point because they *are* the same OSM node - not because they happen to round to the same coordinate bucket. The old coordinate-proximity behaviour is kept only as a fallback for vertices with no known node ID (synthetic boundary-ring points, fixture/demo data, or an Overpass response that for some reason omitted node refs).
...
1. A JSTS `GeometryFactory` is built with a `PrecisionModel` whose scale is `1 / snappingToleranceMeters` - this mainly gives the overlay algorithm numerical robustness and rounds *newly computed intersection points* to the tolerance grid; it does **not** move vertices that were not part of any computed intersection (verified empirically - a JSTS union leaves non-intersecting original vertices bit-for-bit unchanged; see "OSM node identity" below).
...
6. Each resulting edge's start/end coordinate is separately matched, by exact coordinate lookup, back to the real OSM node ID of the original way vertex it came from (when the input line carried one - see "OSM node identity" below), and recorded as `NodedEdge.startNodeId`/`endNodeId`. A coordinate that doesn't match any original vertex is a genuinely new intersection point, so it correctly gets no node ID.
...
Overpass is queried with `out body geom` (see `docs/overpass.md`), so every downloaded way carries a `nodes` array - the real OSM node ID at each vertex, in the same order as its coordinates (`OSMWay.nodeIds`). This survives reprojection and area-clipping (`workers/analysisPipeline.ts::buildNodeIdLookup`, matched by exact projected coordinate, since clipping only drops vertices or inserts brand-new ones at the cut point - it never moves a surviving vertex) and reaches `nodeRoadNetwork` as `NodingInputLine.nodeIds`.

This matters because it is the only reliable way to tell "these two vertices are the same real intersection" apart from "these two vertices simply happen to be close together" - two unrelated features occasionally do end up within a few metres of each other (most commonly right where a bridge or tunnel begins - see step 4), and blind coordinate-proximity snapping cannot distinguish the two cases. Node IDs can.

Synthetic geometry that was never a real OSM way - the boundary-ring closure line (step 5), fixture/demo data, or an Overpass response that for some reason lacked `nodes` - simply has no node IDs, and its vertices fall back to the pre-existing coordinate-proximity behaviour everywhere downstream (see step 7).

## 7. Graph construction

`geometry/graph.ts::buildGraphFromNodedEdges` turns noded edges into an undirected graph: each edge's first/last coordinate becomes (or reuses) a node. **When an endpoint carries a known OSM node ID** (`NodedEdge.startNodeId`/`endNodeId` - see step 6), it is keyed by that ID exactly, so two endpoints are the same graph node if and only if they really are the same OSM node - regardless of how far apart their (possibly imprecise) coordinates are, and *never* merged just because two different, known IDs happen to round to the same coordinate bucket. Only endpoints with no known ID fall back to the original behaviour: keyed by coordinates rounded to the snapping tolerance. Since noding has already split every same-level intersection, this is a direct 1:1 mapping from noded edges to graph edges - no further splitting is needed here.
...
- **`UnaryUnionOp.union()`, even under a coarse `PrecisionModel`, does not snap two non-intersecting nearby vertices together.** It was tempting to assume the noding factory's tolerance-scaled `PrecisionModel` was what made "roads within a few metres of each other" connect - it isn't. Verified empirically (see git history around the OSM-node-identity work): two endpoints that never actually cross stay exactly where they were, unrounded, no matter how coarse the precision model is; only a *newly computed intersection point* gets rounded to the tolerance grid. The real "bridge small gaps" mechanism was entirely in `graph.ts`'s coordinate-bucket rounding, one step later - which is also exactly why that step, not noding, needed to become OSM-node-ID-aware to fix the false-connection edge case in step 4.
