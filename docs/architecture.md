...
│  DistrictPanel  ExportPanel  ErrorPanel  SavedAnalysesPanel          │
└───────────────┬───────────────────────────────────────────────────--┘
                │ reads/writes
┌───────────────▼───────────────────────────────────────────────────--┐
...
`IndexedDbAnalysisCache` uses one IndexedDB database (`urban-blocks-builder-cache`) with separate object stores for cell acquisition results (`cellData` - each entry holds both the road/waterway/railway network and the building points fetched in the same Overpass call), completed-analysis snapshots (`analysisSnapshots`, see below), and last-updated metadata. The cache key for a cell (`buildCellCacheKey`) is a SHA-256 hash (via `crypto.subtle`, falling back to a deterministic non-cryptographic hash if unavailable) of the cell bbox plus **the exact Overpass query text for that cell**, query version, algorithm version, and endpoint. Keying on the query text itself, rather than enumerating individual filters, means any toggle that changes what gets requested (highway types, waterway/railway/building inclusion, access exclusions...) invalidates stale cache entries automatically - an earlier version listed filters individually and missed several when new toggles were added later.

**Saved analyses ("resume without recomputing")**: whenever a non-fixture-mode analysis completes, `workers/geometry.worker.ts` builds an `AnalysisSnapshot` (the area, config, districts/strategy, the final blocks, all four road-network layers, the grid, district statistics, and the report - everything `App.tsx`'s Zustand store needs to render the result) and saves it via `AnalysisCache.saveAnalysisSnapshot`, fire-and-forget (a save failure only surfaces a warning - the completed result the user is looking at is unaffected). `components/SavedAnalyses/SavedAnalysesPanel.tsx`, shown on the area-selection step, lists saved analyses (`listAnalysisSnapshots` - summary fields only, not the full geometry) and, on "Load", calls `loadAnalysisSnapshot` and writes the result straight into the store's setters - no worker involved, nothing re-downloaded or recomputed. At most 10 snapshots are kept; saving an 11th prunes the oldest. This replaced an earlier, never-wired-up version of the same idea (`saveFinalBlocks`/`loadFinalBlocks`/`saveReport`/`loadReport`/`listCachedAnalyses`/`clearAnalysis`, declared on `AnalysisCache` but never called from anywhere) - the old IndexedDB stores backing them (`gridState`, `finalBlocks`, `reports`) are dropped on database upgrade (`DB_VERSION` 2 -> 3) since they never held real data.
